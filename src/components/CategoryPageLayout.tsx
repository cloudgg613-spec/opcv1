...
export default function CategoryPageLayout({
  ...
  return (
    <main className="min-h-screen bg-black text-slate-100 pb-20">
      ...
      <section className="relative w-full h-[180px] sm:h-[320px] bg-black flex items-center overflow-hidden border-b border-slate-800">
        <Image
          src={bannerImage}
          alt={title}
          fill
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full space-y-1.5 sm:space-y-3">
          <h1 className="text-xl sm:text-4xl font-extrabold text-white tracking-tight leading-tight">
            {title}
          </h1>

          <p className="text-slate-300 text-xs sm:text-base max-w-2xl leading-relaxed line-clamp-2 sm:line-clamp-none">
            {subtitle}
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8 space-y-4 sm:space-y-6">
        <div className="p-3.5 sm:p-6 rounded-2xl sm:rounded-3xl bg-[#09090b] border border-slate-800/90 shadow-xl space-y-4 sm:space-y-6">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-4 pb-3.5 sm:pb-6 border-b border-slate-800">
            {/* Filter Title & Counter */}
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="p-2 sm:p-2.5 rounded-xl bg-slate-900 text-slate-300 border border-slate-800 shrink-0">
            ...
            <div className="relative w-full lg:w-80">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 sm:py-2.5 rounded-xl bg-slate-900 border border-slate-700/80 text-slate-100 placeholder-slate-500 text-xs sm:text-sm focus:outline-none focus:border-slate-500 transition-all"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 items-center">
            ...
            <div className="space-y-1.5 sm:space-y-2">
              ...
              <select
                ...
                className="w-full px-3 py-1.5 sm:py-2 rounded-xl bg-slate-900 border border-slate-700/80 text-slate-200 text-xs font-semibold focus:outline-none focus:border-slate-500 transition-all"
              >
        ...
        {loading ? (
          ...
          <div className="p-8 sm:p-12 text-center bg-[#09090b] border border-slate-800 rounded-2xl sm:rounded-3xl space-y-4">
            <XCircle className="w-10 h-10 sm:w-12 sm:h-12 text-slate-500 mx-auto" />
            <h4 className="text-base sm:text-lg font-bold text-slate-200">Không Tìm Thấy Sản Phẩm Phù Hợp</h4>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2.5 sm:gap-6">
            {filteredProducts.map((product) => {
              ...
              return (
                ...
                >
                  {/* Card Header & Image */}
                  <div className="relative aspect-square w-full bg-slate-900/80 overflow-hidden flex items-center justify-center p-2 sm:p-4">
                    <Image
                      src={displayImage}
                      alt={product.name}
                      fill
                      className="object-contain p-2.5 sm:p-4 group-hover:scale-105 transition-transform duration-500"
                    />

                    {/* Category Tag (Hidden on mobile to prevent overlap with stock badge) */}
                    <span className="hidden sm:inline-block absolute top-3 left-3 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-slate-900/90 text-slate-200 border border-slate-700 backdrop-blur-md z-10">
                      {product.category === "one-piece" ? "One Piece" : "Pokémon"}
                    </span>

                    {/* Stock Status Badge */}
                    <span
                      className={`absolute top-2 right-2 sm:top-3 sm:right-3 px-2 py-0.5 sm:px-2.5 sm:py-0.5 rounded-md text-[9px] sm:text-[10px] font-extrabold border z-10 shadow-sm ${
                        product.inStock
                    ...
                    <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-xs z-20">
                      <span className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-slate-800 hover:bg-slate-700 text-white border border-slate-600 font-bold text-xs shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                  ...
                  <div className="p-3 sm:p-5 space-y-2 sm:space-y-3 flex-1 flex flex-col justify-between">
                    ...
                    <div className="pt-2 sm:pt-3 border-t border-slate-800/80 flex items-center justify-between gap-1">
                      ...
                      <span className="hidden sm:inline-block text-[11px] font-semibold text-slate-400 group-hover:text-white underline transition-colors shrink-0">
                        Chi tiết →
                      </span>
                      <span className="sm:hidden text-slate-400 group-hover:text-white text-xs font-bold shrink-0">
                        →
                      </span>
                    </div>
                  </div>
